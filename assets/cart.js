console.log('[cart.js] Script loaded, defining CartItems');
...
class CartItems extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Initialize scroll indicator for cart drawer
  ...
  initScrollIndicator() {
    this.scrollHandler = () => this.updateScrollIndicator();
    ...
    // Check on resize
    const resizeObserver = new ResizeObserver(() => this.updateScrollIndicator());
    resizeObserver.observe(this);
  }
  
  updateScrollIndicator() {
    const hasScrollableContent = this.scrollHeight > this.clientHeight;
    const isScrolledToBottom = this.scrollTop + this.clientHeight >= this.scrollHeight - 5; // 5px threshold
    
    if (hasScrollableContent && !isScrolledToBottom) {
      this.classList.add('has-scroll-indicator');
    } else {
      this.classList.remove('has-scroll-indicator');
    }
  }
    ...
    if (this.tagName === 'CART-DRAWER-ITEMS') {
      return fetch(`${routes.cart_url}?section_id=cart-drawer`)
        ...
        .then((responseText) => {
          ...
          // Reinitialize gift note toggle after replacing elements
          const cartDrawer = document.querySelector('cart-drawer');
          if (cartDrawer && typeof cartDrawer.initGiftNoteToggle === 'function') {
            cartDrawer.initGiftNoteToggle();
          }
          
          // Reinitialize scroll indicator
          const newCartDrawerItems = document.querySelector('cart-drawer-items');
          if (newCartDrawerItems && typeof newCartDrawerItems.initScrollIndicator === 'function') {
            newCartDrawerItems.initScrollIndicator();
          }
        })
        .catch((e) => {
          console.error(e);
        ...
        CartPerformance.measure(`${eventTarget}:paint-updated-sections"`, () => {
          ...
          this.getSectionsToRender().forEach((section) => {
            ...
            if (elementToReplace) {
              ...
              console.warn('Element to replace not found:', section.id, section.selector);
            }
          });
          
          // Reinitialize gift note toggle after updating sections  
          // cartDrawerWrapper already declared above
  ...
  getSectionInnerHTML(html, selector) {
    if (!html) {
      console.error('getSectionInnerHTML - html is null or undefined', { html, selector });
      return '';
    }
    const parsedDoc = new DOMParser().parseFromString(html, 'text/html');
    const element = parsedDoc.querySelector(selector);
    if (!element) {
      console.error('getSectionInnerHTML - selector not found', { selector, html: html.substring(0, 200) });
      return '';
    }
